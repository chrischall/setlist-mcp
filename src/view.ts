 ...
 */
export const SETLIST_VIEWS = ['compact', 'full'] as const;

const NOTE =
  'compact strips image/avatar URLs from the response; "full" returns Setlist.fm\'s payload untouched. ' +
  'No field projection: this server has no verified record of which Setlist.fm fields matter, and inventing ' +
  'one would risk dropping a field a caller needs.';

/** The `view` parameter every read tool in this server takes. */
export const viewArg = (): ReturnType<typeof viewParam> => viewParam(SETLIST_VIEWS, { note: NOTE });
...
export function viewResponse(view: string | undefined, data: unknown): ReturnType<typeof minifiedResult> {
  const rung: View = resolveView(view, SETLIST_VIEWS);
