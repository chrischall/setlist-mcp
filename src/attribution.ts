...
// setlist/artist/venue object carries a `url`, and it survives BOTH view rungs
// — `full` is untouched, and compact's `stripMediaUrls` removes only
// media-named keys and image-extension URLs, neither of which `url` is
// (`tests/view.test.ts` pins that). So this note — appended to each data tool's
// description, but not to setlist_healthcheck — tells the model to surface that
// `url` as a source link.
