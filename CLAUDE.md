...
- **Authenticated-write robustness** (`web-client.ts` + `attendance.ts`): the website session can get throttled/invalidated by a burst of authenticated writes (the symptom: marks succeed for the first few, then every call fails). `SetlistWebClient.gate()` paces every authenticated request ≥`WRITE_PACE_MS` apart (mirrors the resolve pacer; `lastCallAt=0` ⇒ first call never waits). When the attendance control is missing, `looksLoggedOut(html)` (a `/signin` link) distinguishes a dead session — raised as a distinct `SessionExpiredError` ("re-copy SETLIST_SESSION_COOKIE") — from an ambiguous miss (generic "rate-limited or layout" message). The detector is intentionally conservative: an unmatched page falls back to the generic message, never misreporting a layout change as an expiry.
