...
- **Read-only REST API, but two website writes**: the setlist.fm *REST API* exposes no write endpoints, so the 18 data tools are all GETs. The only mutating tools are `setlist_mark_attended` / `setlist_unmark_attended`, which act against the logged-in website, toggle attendance and verify by re-reading the attended list. They are deliberately **not** confirm-gated: each is a reversible toggle (the other tool undoes it) that affects only your own account, so the 2026-09-24 confirmation-gate audit removed the old `confirm` boolean rather than migrating it to `confirmToken`. Don't add one back — fleet CI rejects a boolean `confirm` input.
